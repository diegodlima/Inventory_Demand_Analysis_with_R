...
View(head(table(data.frame(Channel_Location = paste(train_set$Town, train_set$Canal_ID))[, 1]),50))
# Since there are different channels for the same Town, there's a possibility of some channel be sending
# more product than expected.
...
# Since constraints by channel have not been informed, at this point it will be suggested to integrate the
# total of demand to all channels by sharing the total of sells.
...
str(train_set)
# The IDs (Agencia, Cliente and Producto) could be categoric variables. But, to guarantee that possible
# future data won't be duplicated by using different IDs, it will be considered to use the description
# for each one:
...
# So, the Pareto's Theorem will be considered to sub set the products that represent 80% of the total of
# Demand, Return in units or Return in pesos.
# To choose the best strategy will be chosen the dataset with the lowest number of products and the highest
# potential to reduce the number of returns
...
View(demand_by_product %>% filter(Cum_Perc <= 80))
...
View(return_by_product %>% filter(Cum_Perc <= 80))
...
View(cost_by_product %>% filter(Cum_Perc <= 80))
...
# At this point it's being considered to address through the products which represent about 80% of the returned units
# in a trying to reduce wastes. By focusing on the features with the most returns, it can be reached multiple
# benefits, for example: costs reduction, better use of transports, improvement on the relationship with clients.
...
View(demand_by_town %>% filter(Cum_Perc <= 80))
...
View(return_by_town %>% filter(Cum_Perc <= 80))
...
View(cost_by_town %>% filter(Cum_Perc <= 80))
...
#####################################################################################
# Clients Analysis using the list of towns which represent 80% of the total of returned units of the list_products
...
# Since the number of clients is too high, it will be calculated only the clients which represent 80% of returns
# of the list_towns, keeping the same approach
...
View(head(demand_by_client %>% filter(Cum_Perc <= 80), 50))
...
View(head(return_by_town %>% filter(Cum_Perc <= 80), 50))
...
View(head(cost_by_town %>% filter(Cum_Perc <= 80), 50))
...
# Before define the method that will be used to predict the demand, it was suggested to test if the data set
# is able to be handle by a linear regression.
# To be able, it's important that there are multiple occurrences of combined variables (Product, Town, NombreCliente)
# to train the model and then make the predictions.
# To guarantee a fast and efficient return, it will be considered the lists of products, town and client, compiled
# throughout previous analysis:
# list_products: list of products which represent 80% of the total of returned units
# list_towns: list of towns which represent 80% of the total of returned units from the list_products
# list_clients: list of towns which represent 80% of the total of returned units from the list_towns
...
# Since the number of unique occurrences are too high, it's being considered not appropriate to address the
# dataset through a linear regression
...
View(head(train_set_result))
# Since the Dev_uni_proxima has to do to the returned units per week, it's important to focus on the Town and
# Client. So it's recommended to reduce the mean of returns calculated by week as in the function

# This function returns the combination of Product, Town and Client per week, ordered by the most returned products
# from set which most impacts in about 51% of the returns
...
suggested_demand <- function(){
  result <- distinct(train_set_result %>%
                         group_by(Product, Town, NombreCliente, Semana) %>%
                         mutate(
                         ...
                         ), Town, NombreCliente, Semana, .keep_all = T)
  result <- result[, c(1:4, 10:13)]
  View(head(result[order(-result$Return_mean), ], 50))
  }
suggested_demand()

# This function lists the combination of Town and Client per week, ordered by the highest number of returns
# of the Product which was choosed by the user
return_demand_product <- function(prod){
  result <- train_set %>% filter(Product == prod)
  result_2 <- distinct(result %>%
                         group_by(Town, NombreCliente, Semana) %>%
                         mutate(
                           Return_mean = mean(Dev_uni_proxima),
                           Demand_min = min(Demanda_uni_equil),
                           Demand_max = max(Demanda_uni_equil),
  ...
  result_2 <- result_2[, c(2, 4, 6, 7, 15:18)]
  View(head(result_2[order(-result_2$Return_mean), ], 50))
}
# Example of use for the 2nd more returned product
return_demand_product('Nito BIM 62g 1p')

# Final notes:
# It's been presented variables which can be used to ponder the demand:
# Demand_min: minimum of demand per week
# Demand_max: maximum of demand per week
# Demand_mean: mean of demand per week
# With these numbers, it can be limit the maximum number of products of a specific variable combination,
# and ponder if the orders are reasonable
# With new datas, this script can be run again and as the returns descrease, new combined variables will
# be shown
